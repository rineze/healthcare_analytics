...
# Market multi-select (derived from already-loaded county_map_df)
market_src = map_data  # already filtered by state if applicable
available_markets = sorted(market_src["market_name"].dropna().unique())
selected_markets = st.sidebar.multiselect(
    "Market (optional)",
    available_markets,
)
if selected_markets:
    map_data = map_data[map_data["market_name"].isin(selected_markets)]
    org_data = org_data[org_data["county"].isin(map_data["county"])]

# County multi-select (populated after state + market selection)
...
if selected_counties:
    ...
    geo_label = ", ".join(selected_markets)
...
if not map_display.empty:
    ...
    # Use categorical market colors only when filtered (few markets); national view uses enrollment heatmap
    use_market_color = bool(selected_states or selected_markets)
    ...
    if selected_states or selected_markets:
        fig_map.update_geos(fitbounds="locations", visible=False)
    else:
        fig_map.update_geos(visible=False)

    layout_kwargs = dict(height=520, margin=dict(l=0, r=0, t=0, b=0))
    if use_market_color:
        layout_kwargs["showlegend"] = False
    else:
        layout_kwargs["coloraxis_colorbar"] = dict(title="Enrollment", thickness=15, len=0.6)
...
if len(org_totals) > 0:
    ...
    top_org_share = (org_totals.max() / total_enrollment * 100) if total_enrollment > 0 else 0
else:
    top_org_name = "N/A"
    top_org_share = 0

# Penetration rate (eligibles-weighted average across selected geography)
pen_subset = map_data[map_data["county"].isin(selected_counties)] if selected_counties else map_data
valid_pen = pen_subset.dropna(subset=["penetration_rate", "eligibles"])
eligible_total = valid_pen["eligibles"].sum()
avg_penetration = (
    (valid_pen["penetration_rate"] * valid_pen["eligibles"]).sum() / eligible_total
    if eligible_total > 0 else None
)
...
if not treemap_data.empty:
    ...
    eligible_subset = map_data[map_data["county"].isin(selected_counties)] if selected_counties else map_data
